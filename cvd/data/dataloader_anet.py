...
class DataLoader(data.Dataset):
    def __init__(self, opt, split='training', seq_per_img=5):
        ...
        obj_cls_file = './dataset/vg_object_vocab.txt' # From Peter's repo
    ...
    def __getitem__(self, index):
        ...
        # seg feature raw will have the [num_frames, 2048 (optical flow) | 1024 (rgb resnet)] 
        ...
        # input_seq_shape = [10, 21, 4]
        input_seq = np.zeros([self.seq_per_img, self.seq_length+1, 4])
        input_seq[:,1:] = seq_batch

        # seq_length is set to 20
        # gt shape is [10, 20]
        gt_seq = np.zeros([10, self.seq_length])
        # fill the proper places for the captions
        # We know that the gt_seq will always be a [10, 20]
